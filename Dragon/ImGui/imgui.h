...
struct ImVec3       // added by Project Contributors
{
    float x, y, z;
    
    constexpr ImVec3() : x(0.0f), y(0.0f), z(0.0f) { }
    constexpr ImVec3(float _x, float _y, float _z) : x(_x), y(_y), z(_z) { }
    float& operator[] (size_t idx) { IM_ASSERT(idx < 3); return ((float*)this)[idx]; }
    float operator[] (size_t idx) const { IM_ASSERT(idx < 3); return ((const float*)this)[idx]; }
};
...
struct ImVec4
{
    ...
    //  added by Project Contributors
    constexpr ImVec4(const ImVec3& v3) : x(v3.x), y(v3.y), z(v3.z), w(1.0f) { }
    constexpr ImVec4(const ImVec3& v3, float _w) : x(v3.x), y(v3.y), z(v3.z), w(_w) { }
    
    float& operator[] (size_t idx) { IM_ASSERT(idx < 4); return ((float*)this)[idx]; }
    float operator[] (size_t idx) const { IM_ASSERT(idx < 4); return ((const float*)this)[idx]; }
