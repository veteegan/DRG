...
- (NSData *)data {
    if (!_data) {
        if (self.htmlData) {
            _html = [[NSString alloc] initWithData:self.htmlData encoding:NSUTF8StringEncoding];
            self.contentLength = self.htmlData.length;
        } else if (self.html) {
            _data = [self.html dataUsingEncoding:NSUTF8StringEncoding];
    ...
    if (self.fileName) {
        ...
        _data = [res dataUsingEncoding:NSUTF8StringEncoding];
    }
    return _data;
}
